...
HAL_StatusTypeDef i2c_write(uint8_t slave_addr, uint8_t reg_addr,
	...
	uint8_t x = HAL_I2C_Mem_Write(&hi2c1, slave_addr << 1, reg_addr,
	I2C_MEMADD_SIZE_8BIT, data, length, 10);
	while (x != HAL_OK) {
		x = HAL_I2C_Mem_Write(&hi2c1, slave_addr << 1, reg_addr,
		I2C_MEMADD_SIZE_8BIT, data, length, 10);
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef i2c_read(uint8_t slave_addr, uint8_t reg_addr, uint8_t length,
		uint8_t *data) {
	uint8_t x = HAL_I2C_Mem_Read(&hi2c1, slave_addr << 1, reg_addr,
	I2C_MEMADD_SIZE_8BIT, data, length, 10);
	while (x != HAL_OK) {

		x = HAL_I2C_Mem_Read(&hi2c1, slave_addr << 1, reg_addr,
		I2C_MEMADD_SIZE_8BIT, data, length, 10);
	}
	return HAL_OK;
